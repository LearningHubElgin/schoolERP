    ...
    return (
        <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
            <svg width={size} height={size} viewBox="0 0 120 120" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <circle cx="60" cy="60" r={r} fill="none" stroke="url(#tchPG)" strokeWidth="12"
                ...
                <defs><linearGradient id="tchPG" x1="0%" y1="0%" x2="100%" y2="0%">
...
const AdminTeacherAttendance = () => {
    ...
    const [formData, setFormData] = useState({
        teacher_id: '', date: '', status: 'Present', check_in: '', check_out: ''
    });
    ...
    const fetchHolidayData = async () => {
        try {
            // We use the report endpoint just to get the holidays and weekly schedule
            // Sending a minimal range to reduce server load
            const today = format(new Date(), 'yyyy-MM-dd');
            const res = await fetch(`${API_BASE}/api/admin/attendance-report/teachers?startDate=${today}&endDate=${today}`, { headers });
    ...
    const fetchAttendanceData = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE}/api/admin/attendance/teachers?date=${selectedDate}`, { headers });
            const data = await res.json();
            if (data.success) {
                setStats(data.stats);
                setAttendanceList(data.attendance);
                setAllTeachers(data.attendance.map(r => ({ id: r.teacher_id, name: r.name })));
            }
        } catch (error) {
    ...
    const handleManualEntry = (record = null) => {
        if (record) {
            ...
            setFormData({
                teacher_id: record.teacher_id, date: selectedDate, status: record.status,
                check_in: (record.check_in && record.check_in !== '-') ? record.check_in : '',
                check_out: (record.check_out && record.check_out !== '-') ? record.check_out : ''
            });
        } else {
            setEditingRecord(null);
            setFormData({ teacher_id: '', date: selectedDate, status: 'Present', check_in: '', check_out: '' });
        }
        setShowModal(true);
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`${API_BASE}/api/admin/attendance/manual`, {
                method: 'POST',
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/admin/attendance-report/teachers?startDate=${format(start, 'yyyy-MM-dd')}&endDate=${format(end, 'yyyy-MM-dd')}`, { headers });
            const data = await res.json();
            if (data.success) {
                setReportData(data.report || []);
                // Holidays and weeklySchedule are now fetched on mount
            }
        } catch (error) {
    ...
    const isHolidayDate = (date) => {
        try {
            ...
            // 1. Check custom holidays table (range)
            ...
            const dayIndex = dateObj.getDay(); // 0 for Sunday, 1 for Monday, etc.
            ...
            if (schedule) {
                ...
                // Fallback: Sunday is holiday by default
                if (dayIndex === 0) return true;
            }
        } catch (e) {
            console.error("Error in isHolidayDate:", e);
        }
    ...
    const reportStats = useMemo(() => {
        ...
        filteredReport.forEach(teacher => {
            reportDays.forEach(day => {
                const key = format(day, 'yyyy-MM-dd');
                if (isHolidayDate(key)) return;
                if (key > todayStr) return; // Future date
                const entry = teacher.attendance[key];
                if (entry) {
    ...
    }, [filteredReport, reportDays, holidays, weeklySchedule]);

    const navigateReportPeriod = (dir) => {
        if (reportView === 'daily') {
            const current = parseISO(reportDate);
            const target = dir === 'next' ? addDays(current, 1) : subDays(current, 1);
            setReportDate(format(target, 'yyyy-MM-dd'));
        } else if (reportView === 'weekly') {
            setReportPeriodStart(prev => dir === 'next' ? addWeeks(prev, 1) : subWeeks(prev, 1));
        } else if (reportView === 'monthly') {
            setReportPeriodStart(prev => dir === 'next' ? addMonths(prev, 1) : subMonths(prev, 1));
        }
    };
    ...
    const exportPDF = async () => {
        ...
        const rows = filteredReport.map((teacher, idx) => {
            let p = 0, a = 0;
            const row = [idx + 1, teacher.name, teacher.employee_id || '-', teacher.phone || '-'];
            reportDays.forEach(day => {
                const key = format(day, 'yyyy-MM-dd');
                const entry = teacher.attendance[key];
                if (isHolidayDate(key)) { row.push('H'); return; }
        ...
        doc.save(`Teacher_Attendance_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
    ...
    const exportExcel = async () => {
        ...
        const rows = filteredReport.map((teacher, idx) => {
            ...
            reportDays.forEach(day => {
                const key = format(day, 'yyyy-MM-dd');
                const entry = teacher.attendance[key];
                if (isHolidayDate(key)) { row.push('Holiday'); return; }
                if (entry && (entry.status === 'Present' || entry.status === 'Late')) { row.push('P'); p++; }
                else { row.push('A'); a++; }
            });
            const total = p + a;
        ...
        XLSX.writeFile(wb, `Teacher_Attendance_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
        toast.success('Excel exported!');
    };

    // ── Render status cell for report grid ──
    const renderStatusCell = (teacher, day) => {
        const key = format(day, 'yyyy-MM-dd');
        const entry = teacher.attendance[key];
        const isHoliday = isHolidayDate(key);
        const todayStr = format(new Date(), 'yyyy-MM-dd');

        if (isHoliday) return (
            <td key={key} className="px-1 py-2 text-center border-r border-slate-100 bg-slate-50/50">
                <span className="inline-block w-7 h-7 rounded-lg bg-slate-200 text-slate-500 text-[10px] font-bold leading-7">H</span>
            </td>
        );
            ...
            if (key > todayStr) {
                return (
                    <td key={key} className="px-1 py-2 text-center border-r border-slate-100">
                        <span className="inline-block w-7 h-7 rounded-lg bg-slate-100 text-slate-400 text-[10px] font-bold leading-7">-</span>
                    </td>
                );
            }
            return (
    ...
    return (
        <div className="space-y-6">
            {/* ── Header Banner ── */}
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 p-4 md:p-5 text-white shadow-lg">
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-lg md:text-xl font-bold tracking-tight">📊 Teacher Attendance dashboard</h1>
                        <p className="mt-1 text-violet-100 text-xs md:text-sm">
                            Track daily attendance and generate exportable teacher reports.
                        </p>
                    </div>
                ...
                <div className="absolute bottom-0 right-20 -mb-20 w-60 h-60 rounded-full bg-fuchsia-500 opacity-20 blur-3xl"></div>
            </div>

            {/* ── Main Tab Switcher ── */}
            <div className="flex bg-slate-100 rounded-xl p-1 gap-1 w-fit">
                <button onClick={() => setActiveTab('attendance')}
                    className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 ${activeTab === 'attendance' ? 'bg-violet-600 text-white shadow-md shadow-violet-200' : 'text-slate-500 hover:bg-slate-200'}`}>
                    ✅ Daily Attendance
                </button>
                <button onClick={() => setActiveTab('report')}
                    className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 ${activeTab === 'report' ? 'bg-violet-600 text-white shadow-md shadow-violet-200' : 'text-slate-500 hover:bg-slate-200'}`}>
                    📊 Attendance Report
                </button>
            </div>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
                        <div className="flex items-center gap-2">
                            <span className="text-sm font-bold text-slate-600 mr-1">Select Date:</span>
                            <button
                                onClick={() => {
                                    const current = parseISO(selectedDate);
                                    const target = subDays(current, 1);
                                    setSelectedDate(format(target, 'yyyy-MM-dd'));
                                }}
                                className="w-9 h-9 rounded-xl bg-slate-100 hover:bg-violet-100 text-slate-600 hover:text-violet-600 flex items-center justify-center transition-all text-lg font-bold cursor-pointer"
                                title="Previous Day"
                            >←</button>
                            <input type="date" value={selectedDate}
                                onChange={(e) => setSelectedDate(e.target.value)}
                                className="px-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none bg-slate-50 focus:bg-white transition-all text-sm font-medium cursor-pointer"
                        ...
                        {isHolidayDate(selectedDate) && (
                            <div className="flex-1 bg-amber-50 border border-amber-200 text-amber-700 px-4 py-2 rounded-xl flex items-center gap-2 animate-pulse">
                                <span className="text-lg">🌴</span>
                                <div className="text-sm">
                                    <span className="font-bold">Notice:</span> This date is marked as a <span className="font-black underline decoration-amber-300">School Holiday</span> or Closed Day.
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {stats && (
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                            {[
                                { label: 'Total Teachers', value: stats.total, bg: 'bg-violet-50', border: 'border-violet-100', text: 'text-violet-700' },
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <Card className="shadow-sm border-gray-100">
                                <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                                    <span className="w-1 h-6 bg-violet-600 rounded-full"></span> Status Distribution
                                </h2>
                            ...
                            <Card className="shadow-sm border-gray-100">
                                <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                                    <span className="w-1 h-6 bg-fuchsia-600 rounded-full"></span> Attendance Overview
                                </h2>
                                <div className="flex flex-col sm:flex-row items-center justify-around h-auto md:h-64 gap-6 py-4 md:py-0">
                                    <div className="relative w-32 h-32 md:w-48 md:h-48 rounded-full shadow-inner ring-4 md:ring-8 ring-slate-50 flex-shrink-0"
                                        style={{
                                        ...
                                        <div className="absolute inset-0 m-auto w-20 h-20 md:w-32 md:h-32 bg-white rounded-full flex flex-col items-center justify-center shadow-lg ring-1 ring-slate-100">
                                            <span className="text-2xl md:text-4xl font-black text-slate-800 tracking-tight">{stats.total}</span>
                                            <span className="text-[8px] md:text-[10px] text-gray-400 uppercase tracking-[0.2em] font-black">TOTAL</span>
                                        </div>
                                    </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {loading ? (
                                        ...
                                        <tr><td colSpan="9" className="text-center py-16 text-gray-400 font-bold uppercase text-xs tracking-widest mb-2">📭 No Teachers Registered</td></tr>
                                    ) : attendanceList.map(record => (
                                        <tr key={record.teacher_id} className="hover:bg-slate-50/50 transition-all duration-200 group">
                                            <td className="px-5 py-4 font-bold text-slate-700 whitespace-nowrap border-l-4 border-transparent group-hover:border-violet-600 pl-4">{record.name}</td>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-lg shadow-slate-100/40 p-4 md:p-6">
                        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                            ...
                            <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
                                {['daily', 'weekly', 'monthly'].map(m => (
                                    <button key={m} onClick={() => setReportView(m)}
                                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${reportView === m ? 'bg-violet-600 text-white shadow-md shadow-violet-200' : 'text-slate-500 hover:bg-slate-200'}`}>
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex items-center gap-2">
                                    <button onClick={() => navigateReportPeriod('prev')} className="w-9 h-9 rounded-xl bg-slate-100 hover:bg-violet-100 text-slate-600 hover:text-violet-600 flex items-center justify-center transition-all text-lg font-bold cursor-pointer" title="Previous Date">←</button>
                                    {reportView === 'daily' ? (
                                    ...
                                    <button onClick={() => navigateReportPeriod('next')} className="w-9 h-9 rounded-xl bg-slate-100 hover:bg-violet-100 text-slate-600 hover:text-violet-600 flex items-center justify-center transition-all text-lg font-bold cursor-pointer" title="Next Date">→</button>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                                <div className="relative flex-1 lg:w-48">
                                    ...
                                    <input type="text" placeholder="Search ID or Name" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                                        className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-violet-500 outline-none text-sm transition-all" />
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {[
                                { label: 'Total Staff', value: reportStats.totalStaff, icon: '👥', bg: 'bg-violet-50', border: 'border-violet-200', text: 'text-violet-700' },
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex items-center justify-center gap-8">
                            ...
                            <div className="space-y-3">
                                {[
                                    { w: 'bg-violet-500', l: 'Present', v: reportStats.present },
                                    ...
                                    <thead>
                                        <tr className="bg-slate-50/80 border-b border-slate-100">
                                            ...
                                            {reportDays.map(day => {
                                                ...
                                                return (
                                                    <th key={format(day, 'yyyy-MM-dd')} className={`px-1 py-3 text-[9px] font-bold text-center min-w-[32px] border-r border-slate-200 ${isHoliday ? 'text-slate-400 bg-slate-50' : 'text-slate-500'}`}>
                                                        <div>{format(day, 'EEE').slice(0, 2)}</div>
                                                        <div className="text-[10px] font-black">{format(day, 'dd')}</div>
                                                    </th>
                                                );
                                            })}
                                            <th className="px-3 py-3 text-[10px] font-black text-emerald-700 uppercase tracking-wider text-center bg-emerald-100 border-r border-emerald-200 shadow-sm">Present</th>
                                            <th className="px-3 py-3 text-[10px] font-black text-red-700 uppercase tracking-wider text-center bg-red-100 border-r border-red-200 shadow-sm">Absent</th>
                                            <th className="px-3 py-3 text-[10px] font-black text-violet-700 uppercase tracking-wider text-center bg-violet-100 border-r border-violet-200 shadow-sm">Score %</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {filteredReport.map((teacher, idx) => {
                                            const summary = getStaffSummary(teacher);
                                            return (
                                                <tr key={teacher.teacher_id} className="hover:bg-violet-50/30 transition-colors">
                                                    ...
                                                    <td className="px-3 py-2 sticky left-8 bg-white z-10 border-r border-slate-100">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                <div className="text-xs font-bold text-slate-800 truncate max-w-[120px]">{teacher.name}</div>
                                                                <div className="text-[10px] text-slate-400">{teacher.phone || 'Teacher'}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-2 border-r border-slate-100">
                                                        <span className="text-[10px] font-mono bg-slate-100 text-slate-600 px-2 py-1 rounded font-bold">{teacher.employee_id || '-'}</span>
                                                    </td>
                                                    {reportDays.map(day => renderStatusCell(teacher, day))}
                                                    <td className="px-3 py-2 text-center bg-emerald-50/30 border-r border-emerald-100/50 align-middle">
                                                        <div className="flex justify-center">
                                                            <span className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-sm font-black ring-1 ring-emerald-200 shadow-sm">
                                                                {summary.present}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-2 text-center bg-red-50/30 border-r border-red-100/50 align-middle">
                                                        <div className="flex justify-center">
                                                            <span className="w-8 h-8 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-sm font-black ring-1 ring-red-200 shadow-sm">
                                                                {summary.absent}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-2 text-center bg-violet-50/30 border-r border-violet-100/50 align-middle">
                                                        <div className="flex justify-center">
                                                            <span className={`px-3 py-1.5 rounded-xl text-xs font-black shadow-sm ring-1 ring-inset ${summary.pct >= 75 ? 'bg-emerald-500 text-white ring-emerald-600' : summary.pct >= 50 ? 'bg-amber-400 text-white ring-amber-500' : 'bg-red-500 text-white ring-red-600'}`}>
                                                                {summary.pct}%
                                                            </span>
                                                        </div>
                                                    </td>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <select value={formData.teacher_id} onChange={(e) => setFormData({ ...formData, teacher_id: e.target.value })}
                                    className="w-full px-3 py-2 border rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-violet-500 outline-none transition-all font-medium text-gray-700"
                                    required disabled={!!editingRecord}>
                                    <option value="">Select Teacher</option>
                                    {allTeachers.map(s => (<option key={s.id} value={s.id}>{s.name}</option>))}
                                </select>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-violet-500 outline-none transition-all font-medium text-gray-700" required />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit"
                                    className="flex-1 px-4 py-2.5 bg-violet-600 text-white rounded-xl hover:bg-violet-700 font-semibold shadow-lg shadow-violet-200 transition-all active:scale-95">Save Record</button>
                            </div>
                        </form>
                    </div>
                </div>
