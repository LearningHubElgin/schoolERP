...
const SyllabusManagement = () => {
    ...
    useEffect(() => {
        fetchClasses();
        fetchSyllabus(); // Fetch all on mount
    }, []);
    ...
    const fetchSyllabus = async (classId = null) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const url = classId 
                ? `${API_URL}/api/admin/syllabus/${classId}`
                : `${API_URL}/api/admin/syllabus`;
            const response = await fetch(url, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await response.json();
    ...
    const handleClassChange = (e) => {
        ...
        // setFormData({ ...formData, class: classNumber, subject_id: '' });
        // Use functional state update to ensure we have latest state if needed, though not strictly necessary here
        setFormData(prev => ({ ...prev, class: classNumber, stream_id: '', subject_id: '' }));

        if (classNumber) {
            const selectedClass = classes.find(c => c.class_number == classNumber);
            if (selectedClass) {
                if (isHigherSecondaryClass(classNumber)) {
                    fetchClassStreams(classNumber);
                    setSubjects([]); // Wait for stream selection
                } else {
                    setStreams([]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const token = localStorage.getItem('token');
            const url = editId ? `${API_URL}/api/admin/syllabus/${editId}` : `${API_URL}/api/admin/syllabus`;
            ...
            if (result.success) {
                ...
                setEditId(null);
                // Reset file input manually if needed or just let react handle it
                document.getElementById('fileInput').value = '';

                // Always refresh the list to show the new upload
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_URL}/api/admin/syllabus/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const result = await response.json();
    ...
    return (
        <div className="space-y-6">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-600 p-4 md:p-5 text-white shadow-lg">
                <div className="relative z-10">
                    <h1 className="text-lg md:text-xl font-bold tracking-tight flex items-center gap-2">
                        📚 Syllabus Management
                    </h1>
                    <p className="mt-1 text-blue-100 text-xs md:text-sm">Upload and manage class-wise syllabus</p>
                </div>
                <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 rounded-full bg-white opacity-10 blur-3xl"></div>
                <div className="absolute bottom-0 right-20 -mb-20 w-60 h-60 rounded-full bg-cyan-500 opacity-20 blur-3xl"></div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-1 h-fit">
                    <h2 className="text-xl font-medium text-gray-800 mb-4 flex items-center gap-2">
                        {editId ? <FiEdit className="text-blue-600" /> : <FiUpload className="text-blue-600" />} 
                        {editId ? 'Edit syllabus' : 'Upload syllabus'}
                    </h2>
                    <form id="upload-form-card" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Syllabus Content (Optional)</label>
                            <textarea
                                value={formData.content}
                                onChange={(e) => setFormData({ ...formData, content: e.target.value })}
                                placeholder="Write syllabus content here if you don't have a file..."
                                rows="3"
                                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white resize-y"
                            ></textarea>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Syllabus File (PDF/Image, Optional)</label>
                        ...
                        <div className="flex gap-2">
                            ...
                            {editId && (
                                <Button 
                                    ...
                                    onClick={() => {
                                        setEditId(null);
                                        setFormData({ class: '', stream_id: '', subject_id: '', title: '', content: '', file: null });
                                        document.getElementById('fileInput').value = '';
                                    }} 
                ...
                <Card className="lg:col-span-2">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <h2 className="text-xl font-medium text-gray-800 flex items-center gap-2">
                            <FiBook className="text-cyan-600" /> Manage syllabus
                        </h2>
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none bg-white text-sm font-medium"
                            >
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12 text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                            <FiBook className="w-12 h-12 mx-auto mb-3 text-gray-300 opacity-20" />
                                ...
                                <tbody>
                                    {syllabusList.map((item) => (
                                        <tr key={item.id} className="bg-white border-b hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-cyan-700">
                                                {item.class}
                                            </td>
                                            <td className="px-6 py-4 font-medium text-gray-900">
                                                {item.subject_name}
                                                <span className="block text-xs text-gray-400 font-medium">{item.subject_code}</span>
                                            </td>
                                            <td className="px-6 py-4">{item.title}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-medium text-gray-700">
                                                        {item.uploader_role === 'admin' ? 'School admin' : (item.uploader_name || 'Admin')}
                                                    </span>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <button
                                                        ...
                                                        className="p-2 text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors shadow-sm"
                                                        title="View preview"
                                                    >
            ...
            <Modal
                ...
                size="xl"
                footer={<Button variant="outline" onClick={() => setPreviewModal({ ...previewModal, isOpen: false })}>Close</Button>}
            >
