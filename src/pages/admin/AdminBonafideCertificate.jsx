...
const AdminBonafideCertificate = () => {
    ...
    const fetchFilteredSections = async () => {
        try {
            ...
            // Find the class ID from the class number
            const selectedClass = classes.find(c => String(c.class_number) === String(filters.class));
            if (!selectedClass) return;
            ...
            if (res.data.success) {
                ...
                // Reset section filter if current is not in filtered list
            ...
            if (studentsRes.data.success) {
                ...
                const studentsWithCounts = await Promise.all(
                    studentsRes.data.students.map(async (student) => {
                        try {
                            const certRes = await axios.get(`${API_URL}/api/admin/certificates/count/${student.id}`, { headers });
                            return {
                                ...student,
                                certificate_count: certRes.data.count || 0
                            };
                        } catch {
                            return { ...student, certificate_count: 0 };
                        }
                    })
                );
    ...
    const generateNextCertificateNumber = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/api/admin/certificates/next-number`, {
                headers: { Authorization: `Bearer ${token}` },
            ...
            setCertificateData(prev => ({
                ...prev,
                certificateNumber: `BON-${schoolId}-${year}-${randomNum}`
            }));
        }
    };
    ...
    const handleViewHistory = async (student) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`${API_URL}/api/admin/certificates/history/${student.id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.data.success) {
    ...
    const handleViewDummyCertificate = () => {
        const dummyCert = {
            id: 'dummy',
            certificate_number: 'BON-SCH-YYYY-XXXX',
            student_name: '................................................',
            class: '........',
            section: '........',
            roll_no: '........',
    ...
    const generateCertificatePDF = async () => {
        ...
        try {
            ...
            // 1. Save certificate record to database first
            const saveRes = await axios.post(`${API_URL}/api/admin/certificates`, {
            ...
            // 2. Set data for capture and wait for DOM update
            const certData = {
                ...selectedStudent,
            ...
            // Allow time for the hidden template to render
            await new Promise(resolve => setTimeout(resolve, 500));

            // 3. Capture certificate as image
            ...
            pdf.save(`Bonafide_Certificate_${selectedStudent.roll_no}_${selectedStudent.name}.pdf`);
    ...
    const downloadCertificate = async (certificate) => {
        ...
        try {
            ...
            // 1. Fetch data (skip if it's the dummy preview)
            if (certificate.id === 'dummy') {
                certData = certificate;
            } else {
                const res = await axios.get(`${API_URL}/api/admin/certificates/download/${certificate.id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            ...
            // 2. Set for capture and wait for render
            setCertificateToCapture(certData);
            ...
            pdf.addImage(imgData, 'PNG', 0, 0, imgWidth, imgHeight);
            pdf.save(`Bonafide_${certData.certificate_number}.pdf`);
    ...
    const downloadCertificateAsWord = async (certificate) => {
        ...
        try {
            ...
            headerChildren.push(
                new Paragraph({
                    alignment: AlignmentType.CENTER,
                    children: [new TextRun({ text: school.name.toUpperCase(), bold: true, size: 36, color: "1e3a8a" })],
                    spacing: { after: 120 },
                }),
                new Paragraph({
                    alignment: AlignmentType.CENTER,
                ...
                sections: [{
                    properties: {
                        page: {
                            ...
                            borders: {
                                pageBorderTop: { style: BorderStyle.SINGLE, size: 12, color: "1e3a8a", space: 24 },
                                pageBorderBottom: { style: BorderStyle.SINGLE, size: 12, color: "1e3a8a", space: 24 },
                                pageBorderLeft: { style: BorderStyle.SINGLE, size: 12, color: "1e3a8a", space: 24 },
                                pageBorderRight: { style: BorderStyle.SINGLE, size: 12, color: "1e3a8a", space: 24 },
                            },
                        },
                    },
                    children: [
                        ...
                        new Paragraph({
                            ...
                            children: [
                                new TextRun({ 
                                    ...
                                    size: 32, 
                                    color: "1e3a8a",
                                    underline: { type: "single", color: "1e3a8a" }
                                })
                            ],
                            spacing: { after: 400 },
                        }),
                        ...
                        new Paragraph({
                            children: [
                                ...
                                new TextRun({ text: ` is a bonafide student of this institution.`, size: 28 }),
                            ],
                            spacing: { after: 300 },
                        }),
                        new Paragraph({
                            children: [
                                ...
                                new TextRun({ text: ` and is a regular student of this school.`, size: 28 }),
                            ],
                            spacing: { after: 300 },
                        }),
                        ...(certData.purpose ? [
            ...
            saveAs(blob, `Bonafide_Certificate_${certData.certificate_number}.docx`);
    ...
    const CertificateTemplate = ({ data, school, showBorder = true }) => {
        ...
        return (
            <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ marginBottom: '20mm', lineHeight: '1.8' }}>
                            ...
                            <p style={{ fontSize: '14px', textAlign: 'justify', marginBottom: '8mm' }}>
                                ...
                                {' '}and is a regular student of this school.
                            </p>
                ...
                {!showBorder && (
                    <>
                        {/* Same content without outer border for PDF capture */}
                        <div style={{ textAlign: 'center', marginBottom: '15mm' }}>
                            {school.logo && (
                                <img 
                                    src={`${API_URL}${school.logo}`} 
                            ...
                            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#1e3a8a', marginBottom: '5px', textTransform: 'uppercase' }}>
                                {school.name || 'School Name'}
                            </h1>
                        ...
                        <div style={{ textAlign: 'center', marginBottom: '15mm' }}>
                            <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1e3a8a', borderBottom: '2px solid #1e3a8a', display: 'inline-block', paddingBottom: '5px' }}>
                                BONAFIDE CERTIFICATE
                            </h2>
                        </div>
                        <div style={{ textAlign: 'right', marginBottom: '10mm' }}>
                        ...
                        <div style={{ marginBottom: '20mm', lineHeight: '1.8' }}>
                            <p>This is to certify that <strong>{data.student_name}</strong> son/daughter of <strong>{data.father_name || 'N/A'}</strong> and <strong>{data.mother_name || 'N/A'}</strong> is a bonafide student of this institution.</p>
                            <p>He/She is studying in <strong>Class {data.class}</strong> Section <strong>{data.section}</strong> with Roll Number <strong>{data.roll_no}</strong> and is a regular student of this school.</p>
                        ...
                        <div style={{ textAlign: 'center', marginTop: '10mm', fontSize: '10px', color: '#9ca3af' }}>
                        </div>
                    </>
                )}
            </div>
    ...
    const getStudentStatusBadge = (student) => {
        if (student.certificate_count > 0) {
            return <Badge variant="success">{student.certificate_count} Certificate(s)</Badge>;
        }
        ...
        <div className="space-y-6 pb-8">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-2 text-blue-100 text-sm md:text-base">
                            Generate and manage student bonafide certificates
                        </p>
                    </div>
                    <div>
                        <button 
                        ...
                        >
                            <span>👁️</span> View dummy Bonafide Certificate
                        </button>
                    </div>
                </div>
            ...
            <Card variant="elevated">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {/* Stream/Group - Show only for 11 & 12 */}
            ...
            {/* Certificate History Modal with View Option */}
            <Modal
                isOpen={showHistoryModal}
                onClose={() => setShowHistoryModal(false)}
                title={`Certificate History - ${selectedStudent?.name || ''}`}
                size="lg"
            >
                {certificateHistory.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                        <p className="text-gray-500">No certificates issued for this student yet.</p>
                    </div>
                )}
            </Modal>
...
export default AdminBonafideCertificate;