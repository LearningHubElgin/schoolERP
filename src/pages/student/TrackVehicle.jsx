...
const TrackVehicle = () => {
    ...
    const fetchTrackingData = async () => {
        try {
            ...
            const res = await axios.get(`${API_URL}/api/transport/student/tracking`, {
            ...
            if (res.data.success) {
                ...
                if (res.data.vehicle) {
                    ...
                    setErrorMessage(res.data.message || 'No active tracking data available.');
                }
                if (res.data.schoolLocation) {
                    setSchoolLocation(res.data.schoolLocation);
                }
                setLastRefresh(new Date().toLocaleTimeString('en-IN', timeOptions));
            } else {
                setErrorMessage('Failed to fetch tracking data.');
            }
        } catch (error) {
    ...
    const fetchSchoolSettings = async () => {
        // Students do not have access to admin settings.
        // For now, utilizing the default school location fallback or localStorage if available.
        const storedLat = localStorage.getItem('school_lat');
        const storedLng = localStorage.getItem('school_lng');
        if (storedLat && storedLng) {
            setSchoolLocation({ lat: parseFloat(storedLat), lng: parseFloat(storedLng), name: "School Location" });
        }
    };
    ...
    useEffect(() => {
        ...
        // Get Student's Live Location via Browser Geolocation API
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    console.log("Geolocation error or denied. Falling back to school location.", error);
                },
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-12">
            {/* Header */}
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 rounded-2xl p-6 text-white shadow-lg flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h1 className="text-2xl font-black flex items-center gap-3 tracking-tight">
                        🛰️ My Vehicle Tracking
                    </h1>
                    <p className="text-white/80 mt-1 font-medium">Real-time location monitoring for your assigned route</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="text-right hidden md:block">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-white/50">Last Updated</p>
                        <p className="font-mono font-bold">{lastRefresh}</p>
                    </div>
                    <button
                        onClick={fetchTrackingData}
                        className="bg-white/20 hover:bg-white/30 p-3 rounded-xl transition-all active:scale-95 border border-white/20"
                    >
                        <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
                    </button>
                    <div className="bg-white/10 px-4 py-2 rounded-xl border border-white/20">
                        <span className="text-2xl font-bold">{trackingActive ? '1' : '0'}</span>
                        <span className="ml-2 text-xs font-bold uppercase tracking-widest text-white/70">Online</span>
                    </div>
                </div>
            </div>
            ...
            {loading && !vehicle ? (
                <div className="p-12 text-center">
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Connecting to satellite...</p>
                </div>
            ) : errorMessage && !trackingActive ? (
                <Card className="p-12 text-center bg-slate-50/50 border-dashed border-2 border-slate-200">
                    <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                        <AlertCircle className="w-8 h-8 text-slate-300" />
                    </div>
                    <h3 className="text-lg font-bold text-slate-700">Not Assigned / Not Active</h3>
                ...
                <Card className="p-12 text-center bg-slate-50/50 border-dashed border-2 border-slate-200">
                    ...
                    <h3 className="text-lg font-bold text-slate-700">Vehicle Not En Route</h3>
                    <p className="text-slate-500 text-sm max-w-md mx-auto mt-2">
                        Tracking will become available automatically when the driver begins the trip.
                    </p>
                </Card>
            ) : (
                <div className="flex flex-col xl:flex-row gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                        <Card
                            className={`relative overflow-hidden group transition-all p-0 border-indigo-500 shadow-md ring-2 ring-indigo-200`}
                        >
                            ...
                            <div className="p-6">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 shadow-sm">
                                            {vehicle.type && ['car', 'suv', 'sedan', 'jeep'].some(t => vehicle.type.toLowerCase().includes(t)) ? (
                                                <Car size={24} />
                                            ) : (
                                                <Bus size={24} />
                                            )}
                                        ...
                                        <div>
                                            <h3 className="text-lg font-black text-slate-800">{vehicle.vehicle_no || vehicle.registration_no}</h3>
                                            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                                                <span>{vehicle.type || 'Transport'}</span>
                                                <span>•</span>
                                                <span className="text-indigo-500">{vehicle.route || 'Standard Route'}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-8">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2 font-bold text-slate-700">
                                            ...
                                            {vehicle.driver_name || 'Assigned Driver'}
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                    ...
                                    <div className="space-y-1 col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <div className="flex items-center gap-3 font-mono text-[10px] font-bold text-indigo-400">
                                                <MapPin size={10} />
                                                {vehicle.current_latitude ? Number(vehicle.current_latitude).toFixed(6) : '0.000000'}, {vehicle.current_longitude ? Number(vehicle.current_longitude).toFixed(6) : '0.000000'}
                                            </div>
                                        </div>

                                        <div className="flex items-start justify-between gap-4">
                                            <p className="text-sm font-bold text-slate-700 leading-relaxed flex-1">
                                                {vehicle.current_place_name || "Determining precise address..."}
                                            </p>
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    openInMaps(vehicle.current_latitude, vehicle.current_longitude);
                                                }}
                                        ...
                                        {routeInfo && (
                                            <div className="grid grid-cols-2 gap-4 mt-6 bg-white p-4 rounded-xl border border-indigo-100 shadow-sm animate-in slide-in-from-top-2 duration-300">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-indigo-50 rounded-lg text-indigo-600">
                                                        <Navigation size={18} />
                                                    </div>
                                                    <div>
                                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Distance to You</p>
                                                        <p className="text-lg font-black text-slate-800">{routeInfo.distance} <span className="text-xs font-bold text-slate-500">km</span></p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Est. Arrival</p>
                                                        <p className="text-lg font-black text-slate-800">{routeInfo.duration} <span className="text-xs font-bold text-slate-500">mins</span></p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                                    <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400">
                                        <Clock size={12} />
                                        Last sync: {vehicle.last_location_update ? new Date(vehicle.last_location_update).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }) : 'N/A'}
                                    </div>
                                    <span className="text-[10px] font-bold text-indigo-500 uppercase flex items-center gap-1">
                                        <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                                        {studentLocation ? 'Routing to You' : 'Routing to School'}
                                    </span>
                                </div>
                            </div>
                        </Card>
                    ...
                    {/* Integrated Map View */}
                    <div className="xl:w-1/2 min-h-[500px] xl:h-[600px] bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden shadow-inner relative flex flex-col">
                        <div className="bg-white px-4 py-4 border-b border-slate-200 flex justify-between items-center z-10 shadow-sm">
                            <div className="flex items-center gap-3 font-bold text-slate-700">
                                ...
                                <span>
                                    Tracking: <span className="text-indigo-600 ml-1">{vehicle.vehicle_no || vehicle.registration_no}</span>
                                </span>
                            </div>
                        ...
                        <div className="flex-1 relative w-full bg-slate-200 min-h-[400px] overflow-hidden rounded-b-2xl">
                            {vehicle && vehicle.current_latitude ? (
                                <div className="absolute inset-0 w-full h-full">
                                    <LeafletMap
                                        vehicle={vehicle}
                                        destination={studentLocation || schoolLocation}
                                        logo={schoolLogo}
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 gap-4">
                                    ...
                                    <p className="font-bold uppercase tracking-widest text-xs">Awaiting precise location signal...</p>
                                </div>
                            )}
                        </div>
                    </div>
...
const LeafletMap = ({ vehicle, destination, logo, onRouteUpdate }) => {
    ...
    useEffect(() => {
        if (!window.L || !vehicle.current_latitude || !destination.lat) return;
        ...
        // 2. Destination Marker (You or School)
        const logoUrl = logo ? `${API_URL}${logo}` : '/logo.png';
        const isStudent = destination.isStudent;

        let destHtml = '';
        if (isStudent) {
            destHtml = `
                <div style="display: flex; flex-direction: column; align-items: center; filter: drop-shadow(0 4px 8px rgb(0 0 0 / 0.15));">
                    <div style="background: white; padding: 4px; border-radius: 999px; border: 2.5px solid #10b981; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; position: relative; z-index: 10;">
        ...
        const destIcon = window.L.divIcon({ html: destHtml, className: 'd-marker', iconSize: [0, 0], iconAnchor: [0, 15] });

        if (destinationMarkerRef.current) {
            destinationMarkerRef.current.setIcon(destIcon);
            destinationMarkerRef.current.setLatLng([dLat, dLng]);
            destinationMarkerRef.current.getPopup().setContent(`<b>${destination.name}</b><br/>Lat: ${dLat.toFixed(6)}<br/>Lng: ${dLng.toFixed(6)}`);
        } else {
            destinationMarkerRef.current = window.L.marker([dLat, dLng], { icon: destIcon }).addTo(mapInstance.current);
            destinationMarkerRef.current.bindPopup(`<b>${destination.name}</b><br/>Lat: ${dLat.toFixed(6)}<br/>Lng: ${dLng.toFixed(6)}`).openPopup();
        }
        ...
        const fetchRoute = async () => {
            try {
                const response = await fetch(
                    `https://router.project-osrm.org/route/v1/driving/${vLng},${vLat};${dLng},${dLat}?overview=full&geometries=geojson`
                );
                ...
                const straightPath = [[vLat, vLng], [dLat, dLng]];
                if (polylineRef.current) {
        ...
        const bounds = window.L.latLngBounds([[vLat, vLng], [dLat, dLng]]);
